...
ToolMan._dragsortFactory = {
	...
	makeListSortable : function(list) {
		...
		helpers.map(items, function(item) {
			if(item.getAttribute("isDisabled")=="true") {
				//var dragGroup = ToolMan.drag().createSimpleGroup(item);
				//dragGroup.addTransform(function(coordinate, dragEvent) {
				//	return coordinate.constrainTo(0, 0)
				//})
			}
			else {
	...
	_onDragMove : function(dragEvent) {
		...
		if (moveTo != null) {
			helpers.moveBefore(item, moveTo,"up")
			return
		}
		...
		if (moveTo != null) {
			helpers.moveBefore(item, helpers.nextItem(moveTo, item.nodeName),"down")
			return
		}
	},
