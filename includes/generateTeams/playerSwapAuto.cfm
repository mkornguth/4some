...
<cfset arrPlayerLabels = arrayNew(1)>
<cfset arrPlayerLabels = ["A","B","C","D"]>
                            ...
                            <cfif playerToShuffle neq captainId>
                                <li>
                                    <cfoutput>
                                        ...
                                        <cfset playerLabel = arrPlayerLabels[ListFind(teamCurrent, playerToShuffle)]>

                                        <!--- find player to switch with >--->
                                        <cfset playerIdToSwap =  ListGetAt(arrCurrentMatch[newTeam], ListFind(teamCurrent, playerToShuffle))>


                                        #playerLabel# on team #teamId#  <--> #playerLabel# on team #newTeam#

                                        <!--- TODO: check if player to swap with was on captain's team the day before --->
                                        <cfquery datasource="#variables.DSN#" name="switchPlayers">
                                            update match_teams
                                            set shuffled_id = 1
                                                , player_id = CASE player_id WHEN #playerToShuffle# THEN #playerIdToSwap#
                                                                   WHEN #playerIdToSwap# THEN #playerToShuffle# END
                                            where  match_id = #url.mid# and player_id IN (#playerToShuffle#,#playerIdToSwap#)
                                        </cfquery>
